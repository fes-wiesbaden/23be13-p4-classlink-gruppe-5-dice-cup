...
export const authInterceptor: HttpInterceptorFn = (req, next) => {
    ...
    return ensureFreshToken(auth).pipe(
        switchMap((didRefresh) => {
            ...
            const authorized = attachToken(preparedRequest, auth.getEffectiveAccessToken());
...
function ensureFreshToken(auth: AuthService): Observable<boolean> {
    const currentToken = auth.getEffectiveAccessToken();
    if (!currentToken) {
        return of(false);
    }
...
// Stein ist super :D
    if (error.status === 401 && shouldAttemptRetry(request, auth)) {
        return auth.refreshTokens().pipe(
            switchMap(() => {
                const freshToken = auth.getEffectiveAccessToken();
                if (!freshToken) {
